...
class RegHead_2branch(nn.Module):
    def __init__(self,
                 in_unit: int = 368,
                 out_unit_l: int = 6,
                 out_unit_r: int = 6,
                 output_size: tuple = (1, 1),
                 drop_ratio: float = 0.25):
        super(RegHead_2branch, self).__init__()
        self.pool = nn.AdaptiveAvgPool2d(output_size)
        ...
        self.fc_l = nn.Linear(in_features=in_unit, out_features=out_unit_l)
        self.fc_r = nn.Linear(in_features=in_unit, out_features=out_unit_r)
...
class RegNet_2branch(nn.Module):
    def __init__(self,
                 cfg: dict,
                 in_c: int = 3,
                 num_classes_l: int = 6,
                 num_classes_r: int = 6,
                 zero_init_last_bn: bool = True):
        super(RegNet_2branch, self).__init__()
        ...
        # RegHead
        self.head = RegHead_2branch(in_unit=input_channels, out_unit_l=num_classes_l, out_unit_r=num_classes_r)
...
def create_regnet_2branch(model_name, num_classes_l, num_classes_r):
    model_name = model_name.lower().replace("-", "_")
    model = RegNet_2branch(cfg=model_cfgs[model_name], num_classes_l=num_classes_l, num_classes_r=num_classes_r)
    return model
