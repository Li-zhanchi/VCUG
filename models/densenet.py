...
class DenseNet_twoBranch(nn.Module):
    def __init__(self,
                 growth_rate: int = 32,
                 block_config: Tuple[int, int, int, int] = (6, 12, 24, 16),
                 num_init_features: int = 64,
                 ...
                 num_classes_r: int = 6,
                 memory_efficient: bool = False):
        super(DenseNet_twoBranch, self).__init__()
        ...
        # fc layer
        self.classifier_l = nn.Linear(num_features, num_classes_l)
        self.classifier_r = nn.Linear(num_features, num_classes_r)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = F.adaptive_avg_pool2d(out, (1, 1))
        out = torch.flatten(out, 1)
        out_l = self.classifier_l(out)
        out_r = self.classifier_r(out)
        return out_l, out_r
...
def densenet161_twoBranch(**kwargs: Any) -> DenseNet_twoBranch:
    return DenseNet_twoBranch(growth_rate=48,
                    block_config=(6, 12, 36, 24),
                    num_init_features=96,
                    **kwargs)


def load_state_dict_densenet(model: nn.Module, weights_path: str) -> None:
