...
from models.getmodels import get_model_2branch, get_model_2branch_631
from datasets import load_data_vcug2branch_kSplit, load_data_vcug_2branch_631
...
def train():
    ...
    epochs = 300 #300
    batch_size = 4
    vcug_dir = '/home/tanzl/data/VCUG/trainTest/doubleBranch/'
    ...
    net, optimizer, scheduler, model_name = get_model_2branch_631(model_name=model_name, step_szie=90)
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_loader):
            images, labels_l, labels_r = data #应该是label_l, label_r
            optimizer.zero_grad()
        ...
        with torch.no_grad():
            for val_data in validate_loader:
                images, labels_l, labels_r  = val_data  # 左右分支
                outputs_l, outputs_r = net(images.to(device))
                predict_y_l = torch.max(outputs_l, dim=1)[1]
                predict_y_r = torch.max(outputs_r, dim=1)[1]
                acc += (torch.eq(predict_y_l, labels_l.to(device)).sum().item() + torch.eq(predict_y_r, labels_r.to(device)).sum().item())
        ...
        if val_accurate > best_acc:
            ...
            best_epoch = epoch
            save_path = '/home/tanzl/code/VCUG_retrain/result0720_2branch_631/{}_best.pth'.format(model_name)
            torch.save(net.state_dict(), save_path) 
        save_path = '/home/tanzl/code/VCUG_retrain/result0720_2branch_631/{}_last.pth'.format(model_name)
        torch.save(net.state_dict(), save_path) 
